...
Configure `NUXT_PUBLIC_PORTAL_API_BASE_URL` and allow the website origin in Portal CORS. The static host needs its existing `try_files $uri $uri/ /200.html` fallback for event detail routes. The `/webinars` catalogue has a canonical URL and sitemap entry and remains indexable. Individual `/webinars/:id` pages receive `X-Robots-Tag: noindex, nofollow` from nginx on the initial response and matching client metadata. Their event details update in the browser; static hosting cannot emit fresh event HTML or a true HTTP 404 without a rendering service. No restricted event or recording is baked into website output.
